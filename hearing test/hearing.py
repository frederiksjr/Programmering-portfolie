...
class ToneThread(threading.Thread):
    def ___init__(self, pitch, rate=44100, period=0.1):
        super().__init__()
...
# sample_rate = 44100          #sampling rate, Hz, has to be an integer
sample_rate = 128000           #sampling rate, Hz, has to be an integer
duration = 0.1                 # in seconds, can be float if ya want
pitch_hertz = 440.0            # sine frequency, Hz, can be float if ya want
...
def set_callback():
    ...
    # Force it to display pretty with ".0" cuz it looks better
...
button_reset = tkinter.Button(root, text="Reset to 440.0 Hz", command='reset_callback')

entry_set = tkinter.Entry(root, textvariable=set_string, width=20)
label_set = tkinter.Label(root, textvariable="Frequency in Hertz:")
...
entry_hertz.grid(row=0, column=1, sticky=tkinter.W, padx=5, pady=(10,5))
button_reset.grid(row=0, column=2, sticky=tkinter.W, padx=5, pady=(10,5))

label_set.grid(row=1, column=0, sticky=tkinter.W, padx=5, pady=(10,5))
entry_set.grid(row=1, column=1, sticky=tkinter.W, padx=5, pady=(10,5))
button_set.grid(row=1, column=2, sticky=tkinter.W, padx=5, pady=(10,5))
