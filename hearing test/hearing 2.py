    ...
    def run(self):
        while True:
            if self.stopped:
                ...
                sample = make_sin_sample(
                    self.sample_rate, self.duration, self.pitch.value)
...
button_decrease = tkinter.Button(
    root, text="Decrease", command=decrease_callback)
button_freeze = tkinter.Button(root, text="Freeze", command=freeze_callback)
button_increase = tkinter.Button(
    root, text="Increase", command=increase_callback)
...
button_reset = tkinter.Button(
    root, text="Reset to 440.0Hz", command=reset_callback)
...
button_reset.grid(row=0, column=2, sticky=tkinter.W,
                  padx=(5, 10), pady=(10, 5))
